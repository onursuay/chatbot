      ...
      >
        ...
        <nav className="flex-1 px-2.5 py-2 overflow-y-auto overflow-x-hidden">
          {NAV_ITEMS.map((group, groupIndex) => {
            ...
            return (
              <div key={group.group} className={groupIndex > 0 ? "mt-1" : ""}>
                {collapsed ? (
                  <div className="h-px bg-gray-200/60 mx-2 my-3" />
                ) : (
                  <button
                  ...
                  >
                    <span className={`text-[11px] font-bold uppercase tracking-[0.08em] transition-colors ${
                      hasActiveItem ? "text-gray-700" : "text-gray-400 group-hover:text-gray-500"
                    }`}>
                      {group.group}
                    </span>
                    <svg
                      ...
                      className={`w-3 h-3 transition-all duration-200 ${
                        isOpen ? "rotate-180" : ""
                      } ${hasActiveItem ? "text-gray-400" : "text-gray-300 group-hover:text-gray-400"}`}
                    >
                      <path d="M6 9l6 6 6-6" />
                    </svg>
                  </button>
                ...
                {(collapsed || isOpen) && (
                  <div className={`space-y-0.5 ${collapsed ? "" : "mt-0.5 mb-1"}`}>
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-center gap-2.5 rounded-lg text-[13px] transition-all duration-150 relative ${
                            collapsed ? "justify-center px-2 py-2.5" : "px-3 py-[7px] ml-1"
                          } ${
                            active
