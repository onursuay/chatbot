  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-white">{t("automation_rules")}</h2>
      ...
      {showCreate && (
        <div className="bg-dark-900 border border-dark-800 rounded-xl p-6 mb-6">
          <h3 className="text-white font-medium mb-4">{t("new_automation_form")}</h3>
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-dark-400 mb-1">{t("rule_name")}</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-dark-400 mb-1">{t("trigger")}</label>
              ...
              <div>
                <label className="block text-sm text-dark-400 mb-1">{t("action")}</label>
            ...
            {form.trigger_type === "keyword" && (
              <div>
                <label className="block text-sm text-dark-400 mb-1">{t("keywords_comma")}</label>
            ...
            {(form.action_type === "send_message") && (
              <div>
                <label className="block text-sm text-dark-400 mb-1">{t("message_to_send")}</label>
            ...
            {form.action_type === "add_tag" && (
              <div>
                <label className="block text-sm text-dark-400 mb-1">{t("tag_to_add")}</label>
      ...
      {loading ? <p className="text-dark-400 text-sm">{t("loading")}</p> : automations.length === 0 ? (
        <div className="bg-dark-900 border border-dark-800 rounded-xl p-12 text-center">
          <p className="text-dark-400">{t("no_automations")}</p>
          <p className="text-dark-600 text-sm mt-1">{t("no_automations_desc")}</p>
        </div>
      ) : (
            ...
            <div key={a.id} className="bg-dark-900 border border-dark-800 rounded-xl p-5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => deleteAutomation(a.id)}
                  className="text-dark-600 hover:text-red-400 transition text-sm ml-2">{t("delete")}</button>
              </div>
            </div>
          ))}
        </div>
